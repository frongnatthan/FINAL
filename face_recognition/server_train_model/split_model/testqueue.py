...
def train(name,train_dir, model_save_path, n_neighbors=None, knn_algo='ball_tree', verbose=False):
    ...
    n=len(os.listdir(os.path.join(train_dir, name)))


        # Loop through each training image for the current person
    for img_path in image_files_in_folder(os.path.join(train_dir, name)):
        ...
        i += 1
    i=1
    print("folder : "+str(name)+" finished")
    shutil.rmtree("PicForTrain/"+name)
...
class Upload(Resource):
    ...
    def post(self):
        ...
        file_model=name+'.clf'
        return train(name,"PicForTrain","models/"+file_model, n_neighbors=2)
